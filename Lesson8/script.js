...
const game = {
    ...
    start() {
		...
		start_count = document.getElementById("score-value");
		start_count.innerHTML = 0
		pause_status = false
        board.render();
        snake.render();
        food.render();
    ...
    stop() {
		...
		stop_count = document.getElementById("score-value");
		stop_count.innerHTML = "Игра окончена со счетом: " + Number(stop_count.innerHTML);
		pause_status = true
    ...
    move(event) {
        ...
        if (foundFood !== -1) {
            ...
            /* перерендериваем еду */
            food.render();
			var count = document.getElementById("score-value");
